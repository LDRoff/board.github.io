...
// --- НАЧАЛО ИЗМЕНЕНИЙ: Функция теперь принимает все событие `e` ---
export function handleBrush(state, pos, e) {
    const pressure = e.pressure > 0 ? e.pressure : 0.5; // Используем 0.5 для мыши
    state.layers[state.layers.length - 1].points.push({ ...pos, pressure });
}
// --- КОНЕЦ ИЗМЕНЕНИЙ ---

export function handleEraser(state, pos, redrawCallback) {
...
export function handleShapeDrawing(state, pos, event, redrawCallback) {
    redrawCallback();
    const { ctx, zoom } = state;
    ctx.save();
    ...
    if (state.isDrawing) {
        ...
        if (['rect', 'parallelogram', 'cone', 'parallelepiped', 'pyramid', 'frustum', 'truncated-pyramid'].includes(state.activeTool)) { 
            ctx.beginPath(); 
...
export function handleMultiStepDrawing(state, pos, event, redrawCallback) {
    redrawCallback();
    const { ctx, zoom } = state;
    ctx.save();
    ctx.translate(state.panX, state.panY);
