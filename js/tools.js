...
// --- НАЧАЛО ИЗМЕНЕНИЙ: Удалена неиспользуемая функция handleEraser ---
// Эта функция была дубликатом логики, которая уже реализована в pointerHandlers.js.
// Удаляем ее, чтобы избежать путаницы.
// --- КОНЕЦ ИЗМЕНЕНИЙ ---
...
export function handleMultiStepDrawing(iCtx, state, pos, event) {
    ...
    const isDarkMode = document.body.classList.contains('dark-theme');
    iCtx.strokeStyle = isDarkMode ? 'rgba(255, 255, 255, 0.6)' : 'rgba(0, 0, 0, 0.6)';
