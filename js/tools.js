...
export function handleShapeDrawing(ctx, state, pos, event) {
    ...
    if (state.currentAction === 'drawingCurve' && state.tempLayer) {
        ...
        ctx.restore();
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Принудительно сбрасываем стиль линии после отрисовки превью кривой ---
        ctx.setLineDash([]);
        // --- КОНЕЦ ИЗМЕНЕНИЙ ---
        return;
    }
    ...
    ctx.restore();
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Принудительно сбрасываем стиль линии после отрисовки превью ---
    ctx.setLineDash([]);
    // --- КОНЕЦ ИЗМЕНЕНИЙ ---
}