...
export function handleShapeDrawing(ctx, state, pos, event) {
    ...
    if (state.currentAction && state.currentAction.startsWith('drawing')) {
        ...
        if (state.tempLayer) {
            ...
            // --- НАЧАЛО ИЗМЕНЕНИЙ: Обновляем предпросмотр для усеченной сферы ---
        ...
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Обновляем предпросмотр создания для сфер ---
        else if (state.activeTool === 'ellipse' || state.activeTool === 'sphere' || state.activeTool === 'truncated-sphere') { 
            const rx = Math.abs(end.x - start.x) / 2; 
            const ry = event.shiftKey ? rx : Math.abs(end.y - start.y) / 2; // Shift для пропорциональности
