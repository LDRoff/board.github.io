...
export function drawLayer(ctx, layer, canvasState) {
    ...
    if (layer.lineStyle === 'dashed') {
        ...
        const gap = Math.max(4, layer.lineWidth * 2.5); // Увеличен промежуток
        ctx.setLineDash([dash, gap]);
    } else if (layer.lineStyle === 'dash-dot') {
        const dash = Math.max(4, layer.lineWidth * 2.5);
        const gap = Math.max(4, layer.lineWidth * 2); // Увеличен промежуток
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем поддержку волнистой линии для эллипса ---
    else if (layer.type === 'ellipse') { 
        if (layer.lineStyle === 'wavy') {
            const points = [];
            const numSegments = 72; // Больше сегментов для гладкого эллипса
            ...
            drawWavyPath(ctx, points, true); // true для замкнутого пути
        } else {
            ctx.beginPath(); 
    ...
    // --- КОНЕЦ ИЗМЕНЕНИЙ ---
        ...
        const { cx, cy, r, cutY, cutR, cutRy } = layer; 
