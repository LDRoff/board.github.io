...
function drawWavyPath(ctx, points, closed = false) {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Улучшена логика для тонких линий ---
    const amplitude = Math.max(2, ctx.lineWidth * 1.5); // Минимальная амплитуда 2px
    const wavelength = Math.max(15, ctx.lineWidth * 8); // Минимальная длина волны 15px
    // --- КОНЕЦ ИЗМЕНЕНИЙ ---
    ...
    for (let i = 0; i < pathPoints.length - 1; i++) {
        ...
        for (let j = 1; j <= segments; j++) {
            ...
            // Используем Math.cos для более плавного начала и конца волны
...
function drawLayer(ctx, layer) {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Устанавливаем стиль линии ---
    ctx.setLineDash([]);
    if (layer.lineStyle === 'dashed') {
        const dash = Math.max(4, layer.lineWidth * 2);
    ...
    if (layer.type === 'path') {
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем поддержку волнистой линии для кисти ---
