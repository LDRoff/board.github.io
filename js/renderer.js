...
export function drawLayer(ctx, layer, canvasState) {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Обновлена логика масштабирования пунктиров ---
    ctx.setLineDash([]);
...
function drawSelectionBox(ctx, selectedLayers, canvasState) {
    ...
    const rotationHandleY = box.y + box.height + 20 / zoom;
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(cornerX, rotationHandleY, scaledHalfHandle, 0, 2 * Math.PI);
    ctx.fill();
