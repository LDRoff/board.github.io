...
export function drawLayer(ctx, layer) {
    ...
    ctx.setLineDash([]);
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Обновляем стили линий ---
    if (layer.lineStyle === 'dashed') {
        const dash = Math.max(4, layer.lineWidth * 2.5); // Увеличиваем длину штриха
        const gap = Math.max(4, layer.lineWidth * 2);   // Увеличиваем промежуток
        ctx.setLineDash([dash, gap]);
    } else if (layer.lineStyle === 'dash-dot') {
        const dash = Math.max(4, layer.lineWidth * 2.5);
        const gap = Math.max(4, layer.lineWidth * 1.5);
        // "Точка" в canvas - это очень короткий штрих. 
        // Мы используем 1 (для тонких линий) или чуть больше для толстых.
