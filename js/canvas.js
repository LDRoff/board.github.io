...
export function initializeCanvas(canvas, ctx, redrawCallback, saveState, updateToolbarCallback) {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Отдельная функция для зума ---
    function performZoom(direction, zoomCenter) {
        ...
        // Если центр зума не указан, используем центр видимой области холста
        if (!zoomCenter) {
            const rect = canvas.getBoundingClientRect();
            zoomCenter = { x: rect.width / 2, y: rect.height / 2 };
        }

        // Корректируем панорамирование, чтобы точка под курсором оставалась на месте
    ...
    function finishTextInput() { 
        ...
        if (text) { 
            const fontSize = state.activeLineWidth * 4 + 12; 
            const font = `${fontSize}px sans-serif`; 
            ctx.font = font; 
            const textMetrics = ctx.measureText(text); 
            const newLayer = { type: 'text', text, font, x: parseInt(textInput.style.left, 10), y: parseInt(textInput.style.top, 10), width: textMetrics.width, height: fontSize, color: state.activeColor, id: Date.now(), rotation: 0, pivot: { x: 0, y: 0 } }; 
    ...
    textInput.addEventListener('keydown', (e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); finishTextInput(); } });
    ...
    function startDrawing(e) {
        ...
        if (state.activeTool === 'text') { finishTextInput(); textInput.style.left = `${pos.x}px`; textInput.style.top = `${pos.y}px`; textInput.style.color = state.activeColor; textInput.style.fontSize = `${state.activeLineWidth * 4 + 12}px`; textInput.hidden = false; textInput.focus(); return; }
        finishTextInput(); state.dragStartPos = pos;
    ...
    function draw(e) {
        ...
        switch(state.currentAction) {
            case 'movingPivot': actions.handleMovePivot(state, pos); redrawCallback(); break;
            case 'rotating': actions.handleRotate(state, pos, e); redrawCallback(); break;
            case 'moving': actions.handleMove(state, pos, e); redrawCallback(); break;
            case 'scaling': actions.handleScale(state, pos, e); redrawCallback(); break;
            case 'selectionBox': 
        ...
        } else if (['rect', 'line', 'parallelogram', 'triangle', 'ellipse', 'sphere', 'cone', 'parallelepiped', 'pyramid', 'rhombus', 'trapezoid', 'frustum', 'truncated-sphere', 'truncated-pyramid'].includes(state.activeTool) && state.isDrawing) {
            tools.handleShapeDrawing(state, pos, e, redrawCallback);
        }
        
        if (!state.isDrawing) return;
        if (state.activeTool === 'brush') { tools.handleBrush(state, pos, e); redrawCallback(); } 
        else if (state.activeTool === 'eraser') { tools.handleEraser(state, pos, redrawCallback); }
    }
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Обновляем обработчик колесика ---
    canvas.addEventListener('wheel', (e) => {
        // Убираем проверку на Ctrl/Meta
        e.preventDefault(); 
        const rect = canvas.getBoundingClientRect();
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: "Отдаем" функцию зума наружу ---
