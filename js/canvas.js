...
export function initializeCanvas(canvas, ctx, redrawCallback, saveState, updateToolbarCallback) {
    ...
    function updateCursor(handle) {
        ...
        if (handle) {
            switch (handle) {
                // --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем курсор для новой иконки ---
    ...
    function startDrawing(e) {
        if (e.target.id !== 'drawingBoard') return;
        ...
        if (state.activeTool === 'select') {
            ...
            if (state.selectedLayers.length > 0) {
                ...
                // --- НАЧАЛО ИЗМЕНЕНИЙ: Обрабатываем нажатие на новую иконку ---
                if (state.scalingHandle === 'move') {
            ...
            if (clickedLayer) {
                ...
                // Закомментировано, чтобы перемещение начиналось только с иконки
                // state.currentAction = 'moving';
            } else {
    ...
    function draw(e) {
        if (e.target.id !== 'drawingBoard' && state.currentAction === 'none' && !state.isPanning) return;
        ...
        if (!state.isDrawing && state.currentAction === 'none') { 
            ...
            if (state.selectedLayers.length > 0) { 
                ...
                if (!handle && hitTest.getLayerAtPosition(pos, state.selectedLayers)) {
                    // Убираем курсор перемещения с самого объекта
