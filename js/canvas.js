...
// --- НАЧАЛО ИЗМЕНЕНИЙ: Принимаем debouncedSaveState как новый аргумент ---
export function initializeCanvas(canvas, interactionCanvas, ctx, redrawCallback, saveState, updateToolbarCallback, saveViewStateCallback, debouncedSaveState) {
// --- КОНЕЦ ИЗМЕНЕНИЙ ---
    const iCtx = interactionCanvas.getContext('2d');
    
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем debouncedSaveState в объект callbacks ---
    const callbacks = { redrawCallback, saveState, updateToolbarCallback, saveViewStateCallback, debouncedSaveState };
    // --- КОНЕЦ ИЗМЕНЕНИЙ ---
