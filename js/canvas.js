...
export function initializeCanvas(canvas, interactionCanvas, ctx, redrawCallback, saveState, updateToolbarCallback, saveViewStateCallback) {
    ...
    const handlePointerEndOutside = () => {
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем сброс состояния Spen-ластика ---
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Удалена строка, вызывавшая перезапись localStorage ---
    // saveState(state.layers); // <<<< ЭТА СТРОКА БЫЛА ОШИБКОЙ
    // --- КОНЕЦ ИЗМЕНЕНИЙ ---
