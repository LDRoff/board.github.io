...
export function applyTransformations(layer) {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Исправляем вычисление точки вращения ---
    // Точка вращения (pivotPoint) должна быть статической мировыми координатами.
    // Она вычисляется как центр неповёрнутого объекта плюс смещение pivot.
    // Ранее здесь была ошибка: смещение pivot само вращалось перед использованием,
    // что приводило к неверному центру вращения при "запекании" трансформации.
    const pivotPoint = {
        x: centerX + pivot.x,
        y: centerY + pivot.y,
    };
