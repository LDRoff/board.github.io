...
// --- НАЧАЛО ИЗМЕНЕНИЙ: Логика пространственной сетки ---
const SPATIAL_GRID_CELL_SIZE = 256;

/**
 * Создает пространственную сетку для быстрого поиска объектов.
 * @param {Array<object>} layers - Массив всех слоев.
 * @returns {Map<string, Array<object>>} - Карта, где ключ - 'x_y' ячейки, а значение - массив слоев в ней.
 */
...
/**
 * Получает слои, находящиеся в той же ячейке сетки, что и заданная точка.
 * @param {Map<string, Array<object>>} grid - Пространственная сетка.
 * @param {object} pos - Позиция {x, y}.
 * @returns {Array<object>} - Массив слоев-кандидатов для проверки.
 */
export function getLayersInProximity(grid, pos) {
    if (!grid) return [];
