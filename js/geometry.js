// --- НАЧАЛО ИЗМЕНЕНИЙ: Новая функция для получения реальных габаритов повернутого объекта ---
export function getTransformedBoundingBox(layer) {
    const box = getBoundingBox(layer); // Получаем базовые, неповернутые габариты
    if (!box) return null;
    const rotation = layer.rotation || 0;
    if (!rotation) return box; // Если поворота нет, возвращаем как есть

    // Определяем углы рамки
    const corners = [
        { x: box.x, y: box.y },
    ...
    // Находим реальный центр вращения (с учетом смещения пивота)
    ...
    // Находим новые крайние точки, чтобы создать новую описанную рамку
...
export function getBoundingBox(layer) {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Группируем эллипс и сферы ---
...
// --- НАЧАЛО ИЗМЕНЕНИЙ: getGroupBoundingBox теперь учитывает поворот объектов ---
export function getGroupBoundingBox(layers) {
    if (!layers || layers.length === 0) return null;
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    layers.forEach(layer => {
        const box = getTransformedBoundingBox(layer); // Используем новую функцию
        if (box) {
...
// --- НАЧАЛО ИЗМЕНЕНИЙ: Новая функция для получения "логической" рамки без учета поворотов ---
/**
...
export function getGroupLogicalBoundingBox(layers) {
    ...
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    layers.forEach(layer => {
        const box = getBoundingBox(layer); // Используем базовую функцию без учета поворота
        if (box) {
            minX = Math.min(minX, box.x);
            minY = Math.min(minY, box.y);
            maxX = Math.max(maxX, box.x + box.width);
...
export function isPointOnCurve(point, layer, zoom) {
    ...
    for (let i = 1; i < layer.nodes.length; i++) {
        ...
        // Итерация по сегменту кривой для поиска ближайшей точки
        let min_dist_sq = Infinity;
        for (let t = 0; t <= 1; t += 0.05) { // Проверяем каждые 5% кривой
...
// --- НАЧАЛО ИЗМЕНЕНИЙ: Функция для поиска ближайшей точки на кривой для вставки нового узла ---
