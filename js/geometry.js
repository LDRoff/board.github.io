...
export function isPointInRect(point, rect, eraserRadius = 0, onlyOutline = false, lineWidth = 2) {
    if (!rect) return false;
    const hitOuter = point.x >= rect.x - eraserRadius &&
        point.x <= rect.x + rect.width + eraserRadius &&
        point.y >= rect.y - eraserRadius &&
        point.y <= rect.y + rect.height + eraserRadius;

    if (!hitOuter) return false;
    if (!onlyOutline) return true;

    // Check if it's hitting the stroke/outline specifically
    const innerRadius = Math.max(0, eraserRadius + (lineWidth / 2));
    const hitInner = point.x > rect.x + innerRadius &&
        point.x < rect.x + rect.width - innerRadius &&
        point.y > rect.y + innerRadius &&
        point.y < rect.y + rect.height - innerRadius;
...
export function isPointInEllipse(point, layer, eraserRadius = 0, onlyOutline = false) {
    const { cx, cy } = layer;
    const lineWidth = layer.lineWidth || 2;
    const outerRx = (layer.rx ?? layer.r) + eraserRadius;
    const outerRy = (layer.ry ?? layer.r) + eraserRadius;
    ...
    if (distSqOuter > 1) return false; // Not hitting at all
    if (!onlyOutline) return true;     // Fully filled hit test

    // Check if hitting specifically the outline
    const innerRadius = Math.max(0, eraserRadius + (lineWidth / 2));
    const innerRx = (layer.rx ?? layer.r) - innerRadius;
    const innerRy = (layer.ry ?? layer.r) - innerRadius;

    // If outline logic makes the inner oval negative/zero, then it's all outline
    if (innerRx <= 0 || innerRy <= 0) return true;

    const distSqInner = (dx * dx) / (innerRx * innerRx) + (dy * dy) / (innerRy * innerRy);
    return distSqInner >= 1; // It hits if it's outside the "inner" cutout
}
