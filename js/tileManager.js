...
export class TileManager {
    ...
    _renderTile(key, params, state, drawLayerCallback) {
        ...
        const PADDING = 2;
        const textureSize = Math.min(4096, Math.ceil((TILE_SIZE + PADDING * 2) * scaleFactor * dpr));
        ...
        for (let row = startRow; row <= endRow; row++) {
            for (let col = startCol; col <= endCol; col++) {
                ...
                if (tile) {
                    ...
                    // Чтобы между тайлами не было субпиксельных пустот при масштабировании (белые щели),
                    // используем крошечное перекрытие. Оно берётся из безопасного PADDING (без грязных полупрозрачных краев).
                    const screenOverlap = 0.5 / zoom;
                    const safeOverlap = Math.min(screenOverlap, P - 0.1);

                    ctx.drawImage(
                        tile.canvas,
                        // Источник: берем центр, отступив от краев tile.canvas (исключая грязные края)
                        (P - safeOverlap) * rScale,
                        (P - safeOverlap) * rScale,
                        (TILE_SIZE + safeOverlap * 2) * rScale,
                        (TILE_SIZE + safeOverlap * 2) * rScale,
                        // Назначение: строгая сетка с безопасным микро-перекрытием
