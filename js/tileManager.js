...
export class TileManager {
    ...
    _renderTile(key, params, state, drawLayerCallback) {
        ...
        // Размер текстуры: ограничиваем 2048px чтобы не превышать лимит GPU
        // Добавляем PADDING (2px), чтобы избежать сглаживания краев при масштабировании на границах
        const PADDING = 2;
        const textureSize = Math.min(2048, Math.ceil((TILE_SIZE + PADDING * 2) * scaleFactor * dpr));
        ...
        const realScale = textureSize / (TILE_SIZE + PADDING * 2);
        tCtx.scale(realScale, realScale);
        tCtx.translate(-(col * TILE_SIZE - PADDING), -(row * TILE_SIZE - PADDING));
        ...
        // --- ИЗМЕНЕНИЕ: Всегда перебираем state.layers для сохранения правильного Z-индекса (порядка слоев). ---
        // Если использовать итерацию по Set (candidateSet), порядок элементов сбивается 
        // из-за порядка обхода ячеек сетки, что приводит к неправильному наложению слоев на границах тайлов.
        state.layers.forEach(layer => {
            if (candidateSet && !candidateSet.has(layer)) {
                return; // Пропускаем слои, которых нет в сетке для этого тайла
            }
            ...
            // Skip layers currently being edited — the DOM editor shows them instead.
            if (layer.isEditing) {
                return;
            }

            // Фильтр привязанных к PDF слоев
            if (layer.parentId) {
                const parent = state.layers.find(l => l.id === layer.parentId);
                if (parent && parent.type === 'pdf') {
                    if (parent.currentPage !== layer.parentPage) {
                        return; // Пропускаем, если страница не та
                    }
                }
            }
            ...
            drawLayerCallback(tCtx, layer, { zoom: 1, isTileRender: true });
            renderedLayerCount++;
        });

        console.log(`[TileManager] _renderTile ${key}: foundLayers=${candidateSet ? candidateSet.size : state.layers.length} renderedLayers=${renderedLayerCount}`);
    ...
    drawVisibleTiles(ctx, state, drawLayerCallback) {
        ...
        console.log(`[TileManager] drawVisibleTiles: tiles=${this.tiles.size} queue=${this.renderQueue.size} queued=${queuedCount} isActionActive=${isActionActive} isPanning=${isPanning} isDrawing=${isDrawing} isInteracting=${isInteracting} isMultiTouching=${isMultiTouching} isProcessingQueue=${this.isProcessingQueue} layers=${state.layers.length} spatialGrid=${state.spatialGrid ? state.spatialGrid.size : 'null'}`);
        ...
        for (let row = startRow; row <= endRow; row++) {
            for (let col = startCol; col <= endCol; col++) {
                ...
                if (tile) {
                    const P = tile.padding !== undefined ? tile.padding : 2;
                    const rScale = tile.realScale || (tile.canvas.width / (TILE_SIZE + P * 2));
                    const screenOverlap = 0.5 / zoom;
                    const safeOverlap = Math.min(screenOverlap, P - 0.1);

                    ctx.drawImage(
                        tile.canvas,
                        (P - safeOverlap) * rScale,
                        (P - safeOverlap) * rScale,
                        (TILE_SIZE + safeOverlap * 2) * rScale,
                        (TILE_SIZE + safeOverlap * 2) * rScale,
                        col * TILE_SIZE - safeOverlap,
                        row * TILE_SIZE - safeOverlap,
                        TILE_SIZE + safeOverlap * 2,
                        TILE_SIZE + safeOverlap * 2
                    );
                }
            }
        ...
        if (this.tiles.size > 80) {
            for (const [key, tile] of this.tiles) {
                const [c, r] = key.split('_').map(Number);
                if (c < startCol - 2 || c > endCol + 2 || r < startRow - 2 || r > endRow + 2) {
