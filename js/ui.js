...
export function updateEraserCursor(x, y, zoom, visible) {
    ...
    const size = Math.max(10, 24 * zoom); // Размер зависит от зума, но не слишком мелкий
...
export function updateFloatingToolbar(state) {
    ...
    if (state.isEditingText) {
        ...
        if (format) {
            ...
            if (fontFamilyDisplay && format.fontName) {
                fontFamilyDisplay.textContent = format.fontName.replace(/['"]/g, '');
            }
        }
        return;
    }
    ...
    if (isSingleTextSelection) {
        ...
        if (box) {
            const fontFamilyDisplay = document.getElementById('font-family-display');
            if (fontFamilyDisplay) fontFamilyDisplay.textContent = layer.fontFamily || 'Arial';
