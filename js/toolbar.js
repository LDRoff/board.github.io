...
export function initializeToolbar(canvasState, redrawCallback, updateToolbarCallback, handlers) {
    ...
    function toggleShapeDropdown(button, container) {
        const allContainers = [shapes2DToolContainer, shapes3DToolContainer];
        const otherContainer = allContainers.find(c => c !== container);
        const wasActive = container.classList.contains('active');
        
        allContainers.forEach(c => {
            c.classList.remove('active');
            const dropdown = c.querySelector('.tool-options');
            if (dropdown) dropdown.classList.remove('opens-downward');
        });
    ...
    toolbar.addEventListener('click', (e) => {
        ...
        if (button.id === 'addImageBtn') {
            document.getElementById('imageUpload').click();
            return;
        }
        
        if (button.dataset.toolGroup === 'shapes') return;
    ...
    // --- НАЧАЛО ИСПРАВЛЕНИЯ: Удалена старая, дублирующаяся логика ---
    document.addEventListener('click', (e) => {
