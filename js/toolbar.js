export function initializeToolbar(canvasState, redrawCallback, updateToolbarCallback) {
    ...
    toolbar.addEventListener('click', (e) => {
        ...
        if (tool !== 'select') { canvasState.selectedLayers = []; redrawCallback(); }
    ...
    function showIndicator(e) {
        // Для сенсорных устройств отключаем прокрутку страницы, чтобы можно было двигать ползунок
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Полная замена системы событий для индикатора ---

    // Удаляем старые, отдельные обработчики для мыши и касаний
    // lineWidthSlider.addEventListener('mousedown', showIndicator);
    // lineWidthSlider.addEventListener('touchstart', ...);
    // document.addEventListener('mouseup', hideIndicator);
    // document.addEventListener('touchend', hideIndicator);

    // Используем универсальные события указателя, которые работают для мыши, касаний и пера
