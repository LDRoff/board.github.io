...
export function initializeToolbar(canvasState, redrawCallback, updateToolbarCallback, handlers) {
    ...
    document.querySelectorAll('.size-editor-wrapper, .size-editor-wrapper-mobile').forEach(wrapper => {
    ...
    sliderContainers.forEach(container => {
        ...
        if (button) {
            button.addEventListener('click', e => {
                e.stopPropagation();
                sliderContainers.forEach(c => { if(c !== container) c.classList.remove('active')});
    ...
    document.addEventListener('click', (e) => {
        ...
        sliderContainers.forEach(c => { if (!c.contains(e.target)) c.classList.remove('active'); });
    });
