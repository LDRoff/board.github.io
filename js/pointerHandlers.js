    ...
    if (state.currentAction !== 'none' || state.isDrawing) {
        ...
        if ((state.isDrawing && isShapeTool) || state.currentAction.startsWith('drawing')) {
            ...
            if (state.activeTool === 'brush' || state.activeTool === 'smart-brush') {
                ...
                const points = state.tempLayer.points;

                iCtx.save();
                iCtx.translate(state.panX, state.panY);
                iCtx.scale(state.zoom, state.zoom);
                
                // --- НАЧАЛО ИЗМЕНЕНИЙ: Сбрасываем стиль линии на сплошной ---
                iCtx.setLineDash([]);
                // --- КОНЕЦ ИЗМЕНЕНИЙ ---
                ...
                if (points.length >= 3) {
                    ...
                    const pressure1 = p1.pressure > 0 ? p1.pressure : 0.5;
                    const pressure2 = p2.pressure > 0 ? p2.pressure : 0.5;

                    const midPoint = { x: (p1.x + p2.x) / 2, y: (p1.y + p2.y) / 2 };
                    
                    iCtx.lineWidth = Math.max(0.5, state.tempLayer.lineWidth * pressure1);
                    iCtx.beginPath();
                    iCtx.moveTo(midPoint.x, midPoint.y);
                    iCtx.quadraticCurveTo(p1.x, p1.y, midPoint.x, midPoint.y);
                    iCtx.stroke();
                    
                    iCtx.lineWidth = Math.max(0.5, state.tempLayer.lineWidth * pressure2);
                    iCtx.beginPath();
                    iCtx.moveTo(midPoint.x, midPoint.y);
                    iCtx.quadraticCurveTo(p2.x, p2.y, midPoint.x, midPoint.y);
                    iCtx.stroke();
                    ...
                    iCtx.lineWidth = Math.max(0.5, state.tempLayer.lineWidth * avgPressure);
                    iCtx.beginPath();
                    iCtx.moveTo(points[0].x, points[0].y);
                    iCtx.lineTo(points[1].x, points[1].y);
                    iCtx.stroke();
                }
