...
function prepareInteractionCache(state) {
    ...
    // 1. Подготовка КЭША (Фон + невыделенные объекты)
    // Сбрасываем трансформацию и очищаем физические пиксели
    cacheCtx.setTransform(1, 0, 0, 1, 0, 0);
    cacheCtx.clearRect(0, 0, cacheCanvas.width, cacheCanvas.height);
    
    cacheCtx.save();
    cacheCtx.scale(dpr, dpr); // Учитываем Retina
    ...
    // 2. Очистка ОСНОВНОГО холста (теперь он будет прозрачным, пока идет interaction)
    ctx.setTransform(1, 0, 0, 1, 0, 0);
    ctx.clearRect(0, 0, state.canvas.width, state.canvas.height);

    // 3. Мгновенная отрисовка на INTERACTION холст (Чтобы не было мерцания!)
    ...
    // Рисуем выделенные объекты поверх
    iCtx.save();
    iCtx.scale(dpr, dpr); // Учитываем Retina
    ...
    if (state.activeTool === 'select') {
        ...
        if (state.clickCount === 2 && e.button === 0) {
            ...
            if (clickedLayer && clickedLayer.type === 'text') {
                ...
                // Инвалидируем плитку перед началом редактирования (скроем старый текст)
                ...
                textTool.startEditing(state, clickedLayer, (isIntermediate, before, after) => {
                    ...
                    if (finishedLayer) { 
                        ...
                        // Инвалидируем плитку после завершения (покажем новый текст)
        ...
        if (handle) {
            ...
            // Инвалидация СТАРОГО положения перед началом трансформации
            if (state.tileManager) {
        ...
        if (state.selectedLayers.length > 0) {
            ...
            if (selectionBox) {
                ...
                if (geo.isPointInRect(rotatedPos, logicalBox)) {
                    ...
                    // Инвалидация СТАРОГО положения перед началом движения
                    if (state.tileManager) {
                        state.selectedLayers.forEach(layer => state.tileManager.invalidateLayer(layer, state));
                    }
...
export function draw(state, callbacks, e) {
    ...
    if (state.isInteracting) {
        ...
        // Очистка и отрисовка с учетом DPR
        state.iCtx.setTransform(1, 0, 0, 1, 0, 0);
        state.iCtx.clearRect(0, 0, state.interactionCanvas.width, state.interactionCanvas.height);
        ...
        state.iCtx.save();
        state.iCtx.scale(dpr, dpr); // Учитываем Retina для отрисовки при перемещении
    ...
    if (state.currentAction !== 'none' || state.isDrawing) {
        ...
        switch(state.currentAction) {
            case 'selectionBox': 
                ...
                state.iCtx.clearRect(0, 0, state.interactionCanvas.width, state.interactionCanvas.height);
                state.iCtx.save();
                state.iCtx.scale(dpr, dpr); // Retina
                state.iCtx.translate(state.panX, state.panY);
                state.iCtx.scale(state.zoom, state.zoom);
...
export function stopDrawing(state, callbacks, e) {
    ...
    if (state.currentAction.startsWith('drawing')) {
        ...
        if (state.currentAction === 'drawingParallelogramSlant') { 
            ...
            if (state.tempLayer.width > 5 || state.tempLayer.height > 5) { createdLayer = state.tempLayer; } 
            isFinalStep = true; 
        } 
        else if (state.currentAction === 'drawingTriangleApex') { 
            state.tempLayer.p3 = finalPos; 
            if (Math.abs(state.tempLayer.p1.x - state.tempLayer.p2.x) > 5 || Math.abs(state.tempLayer.p1.y - state.tempLayer.p2.y) > 5) { createdLayer = state.tempLayer; } 
            isFinalStep = true; 
        } 
            ...
            delete state.tempLayer.apex; createdLayer = state.tempLayer; isFinalStep = true; 
        } 
            ...
            state.tempLayer.p4 = finalPos; if (Math.abs(state.tempLayer.p1.x - state.tempLayer.p2.x) > 5) { createdLayer = state.tempLayer; } isFinalStep = true; 
        } 
        else if (state.currentAction === 'drawingFrustum') { 
            const { cx } = state.tempLayer; state.tempLayer.topY = finalPos.y; state.tempLayer.rx2 = Math.abs(finalPos.x - cx); state.tempLayer.ry2 = state.tempLayer.rx2 * 0.3; createdLayer = state.tempLayer; isFinalStep = true; 
        } 
        else if (state.currentAction === 'drawingTruncatedSphere') { 
            const { cx, cy, rx, ry } = state.tempLayer; 
            const cutY = Math.max(cy - ry, Math.min(cy + ry, finalPos.y)); 
            const h = Math.abs(cutY - cy); 
            const ratioY = h / ry; 
            const cutRx = rx * Math.sqrt(1 - (ratioY * ratioY)); 
            state.tempLayer.cutY = cutY; state.tempLayer.cutR = cutRx > 0 ? cutRx : 0; state.tempLayer.cutRy = state.tempLayer.cutR * 0.3; createdLayer = state.tempLayer; isFinalStep = true; 
        }
    ...
    if (transformActions.includes(state.currentAction)) {
        ...
        if (state.currentAction === 'editingCurve' && dragDistance < 5 / state.zoom) {
            ...
            // --- Инвалидация НОВОГО положения ---
            // ВАЖНО: передаем state для обновления BBox cache
            if (state.tileManager) {
                state.selectedLayers.forEach(layer => state.tileManager.invalidateLayer(layer, state));
            }
    ...
    if (state.isDrawing) {
        if (state.activeTool === 'brush' || state.activeTool === 'smart-brush') {
            ...
            if (newLayer && newLayer.points.length > 1) {
                ...
                // ВАЖНО: передаем state
                if (state.tileManager) state.tileManager.invalidateLayer(finalLayer, state);
                ...
                const dpr = window.devicePixelRatio || 1;
                ctx.save();
                ctx.scale(dpr, dpr); // Учитываем Retina
