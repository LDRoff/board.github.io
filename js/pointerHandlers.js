...
export function draw(state, callbacks, e) {
    ...
    const transformActions = ['moving', 'scaling', 'rotating', 'movingPivot', 'editingCurve'];
    if (transformActions.includes(state.currentAction) && !state.isInteracting) {
...
export function stopDrawing(state, callbacks, e) {
    ...
    const { redrawCallback, saveState, updateToolbarCallback, debouncedPerformSaveState } = callbacks;
    ...
    if (transformActions.includes(state.currentAction)) {
        if (state.currentAction === 'rotating') {
            // "Запекаем" групповое вращение в индивидуальные слои
            if (state.selectedLayers.length > 1 && state.groupRotation !== 0) {
                state.selectedLayers.forEach((layer) => {
                    const box = geo.getBoundingBox(layer); // Используем текущий bounding box
                    if (!box) return;
    ...
    if (state.isDrawing) {
        if (state.activeTool === 'brush' || state.activeTool === 'smart-brush') {
            ...
            if (newLayer && newLayer.points.length > 1) {
                ...
                debouncedPerformSaveState({ type: 'creation', before: [], after: [finalLayer] });
      ...
      // Do nothing, redraw is called below
    } else {
        redrawCallback();
    }
