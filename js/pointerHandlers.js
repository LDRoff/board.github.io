...
export function startDrawing(state, callbacks, hideContextMenu, e) {
    ...
    if (state.activeTool === 'brush' || state.activeTool === 'smart-brush') {
        ...
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Проверяем тип слоя перед добавлением в список на удаление ---
        if (layerToErase && layerToErase.type !== 'image') {
    ...
    if (state.isDrawing || state.currentAction !== 'none') {
        ...
        if (state.isDrawing) {
            if (state.activeTool === 'brush' || state.activeTool === 'smart-brush') {
                ...
                const layerToErase = hitTest.getLayerAtPosition(pos, state.layers);
                // --- НАЧАЛО ИЗМЕНЕНИЙ: Проверяем тип слоя при движении ластика ---
                if (layerToErase && layerToErase.type !== 'image' && !state.layersToErase.has(layerToErase)) {
                    state.layersToErase.add(layerToErase);
                    state.didErase = true;
                    redrawCallback();
                }
