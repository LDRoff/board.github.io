...
export function handleMove(state, pos, event) {
    ...
    state.selectedLayers.forEach(layer => {
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем 'text' в список перемещаемых объектов ---
...
export function handleScale(state, pos, event) {
    ...
    state.selectedLayers.forEach((layer, index) => {
        ...
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Логика масштабирования текста ---
        if (['rect', 'image', 'text'].includes(layer.type)) { 
            ...
            // Удалена строка, изменявшая размер шрифта
        }
        // --- КОНЕЦ ИЗМЕНЕНИЙ ---
...
export function handleRotate(state, pos, event) {
    ...
    state.selectedLayers.forEach((layer, index) => {
        ...
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем 'text' в список вращаемых объектов ---
        if (['rect', 'image', 'text', 'parallelogram', 'parallelepiped'].includes(layer.type)) {
