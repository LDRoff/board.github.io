    ...
    state.transformGroup.forEach((layer, index) => {
        ...
        if (layer.type === 'text') {
            ...
            if (!['left', 'right', 'top', 'bottom'].includes(handle)) {
                ...
                // Сбрасываем кеш — он будет перегенерирован с новым fontSize при stopDrawing
                layer.cachedImage = null;
            } else {
                // Reflow при перетаскивании боковых ручек
                layer.cachedImage = null;
                const now = Date.now();
            ...
            // Синхронизируем DOM-редактор, если текст редактируется
            if (layer.isEditing && state.syncTextEditorWithCanvas) {
