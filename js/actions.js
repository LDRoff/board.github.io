...
export function handleMove(state, pos, event) {
    ...
    state.selectedLayers.forEach(layer => {
        if (['rect', 'image', 'text', 'parallelogram', 'parallelepiped'].includes(layer.type)) {
...
export function handleScale(state, pos, event) {
    ...
    state.selectedLayers.forEach((layer, index) => {
        ...
        if (['rect', 'image', 'text'].includes(layer.type)) { layer.x = nX + (originalLayer.x - oBox.x) * scaleX; layer.y = nY + (originalLayer.y - oBox.y) * scaleY; layer.width = originalLayer.width * scaleX; layer.height = originalLayer.height * scaleY; if (layer.type === 'text') { layer.font = `${Math.abs(layer.height)}px sans-serif`; } }
...
export function endSelectionBox(state, pos, event) {
    const selBox = { x: Math.min(pos.x, state.startPos.x), y: Math.min(pos.y, state.startPos.y), width: Math.abs(pos.x - state.startPos.x), height: Math.abs(pos.y - state.startPos.y), };
