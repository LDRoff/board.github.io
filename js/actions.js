...
// Helper для сокращения кода и DRY
function invalidateSelection(state) {
...
export function handleMove(state, pos, event) {
    ...
    // Инвалидация ДО изменения (чтобы стереть старое положение)
    // !!! Мы убрали это, чтобы избежать артефактов при движении.
    // Инвалидация происходит в startDrawing (перед движением) и stopDrawing (после).
    ...
    if (shouldSnap) {
        ...
        if (originalSelectionBox) {
            ...
            const candidates = nearbyLayers.size > 0 ? Array.from(nearbyLayers) : state.layers.filter(l => !selectedIds.has(l.id));
            ...
            for(const gridPoint of gridPoints) {
                ...
                if (Math.abs(diffX) < SNAP_THRESHOLD) {
                    if (bestSnapDX === null || Math.abs(diffX) < Math.abs(bestSnapDX)) bestSnapDX = diffX;
                }
                const snappedY = snapToGrid(gridPoint.y);
                const diffY = snappedY - gridPoint.y;
                 if (Math.abs(diffY) < SNAP_THRESHOLD) {
                    if (bestSnapDY === null || Math.abs(diffY) < Math.abs(bestSnapDY)) bestSnapDY = diffY;
                }
            }
    ...
    // !!! Тоже убрали, так как финальная позиция фиксируется в stopDrawing.
    // invalidateSelection(state);
}

export function handleScale(state, pos, event) {
    // При скейлинге артефактов обычно меньше, так как он реже,
    // но для консистентности лучше тоже убрать мгновенную инвалидацию 
    // и положиться на start/stop. Но так как при скейле меняется размер, 
    // старая картинка на плитке может сильно разъехаться с новой.
    // Однако, мы рисуем поверх в iCtx, поэтому должно быть нормально.
    
    // invalidateSelection(state); // REMOVED
