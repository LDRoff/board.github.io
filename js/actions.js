...
export function handleMove(state, pos, event) {
    // Общее смещение от точки начала перетаскивания (неизменной)
    ...
    if (shouldSnap) {
        ...
        if (originalSelectionBox) {
            // Вычисляем, где бы находился объект без примагничивания, основываясь на движении мыши
            ...
            // Добавляем смещение для примагничивания к общему смещению от мыши
    ...
    // Применяем финальное смещение (с учетом примагничивания) к оригинальным позициям
...
export function handleRotate(state, pos, event) {
    ...
    // Если выделено несколько объектов, мы меняем только общий угол поворота группы
    if (state.selectedLayers.length > 1) {
        state.groupRotation = deltaAngle;
    } 
    // Если объект один, меняем его собственный угол поворота
...
export function handleMovePivot(state, pos) {
    ...
    // 1. Старое положение пивота в мировых координатах
    const oldPivotWorld = {
        x: originalCenterX + (originalLayer.pivot?.x || 0),
        y: originalCenterY + (originalLayer.pivot?.y || 0),
    };

    // 2. Новое желаемое положение пивота в мировых координатах (положение курсора)
    const newPivotWorld = { x: pos.x, y: pos.y };

    // 3. Смещение пивота в мировых координатах
    const pivotDisplacement = {
        x: newPivotWorld.x - oldPivotWorld.x,
        y: newPivotWorld.y - oldPivotWorld.y,
    };

    // 4. Вычисляем компенсирующий сдвиг для объекта.
    // Это сдвиг, который нужно применить к координатам объекта, чтобы он
    // визуально остался на месте после смены центра вращения.
    // Формула: T = (I - R(-rotation)) * pivotDisplacement
    ...
    // 5. Применяем компенсацию к текущим координатам слоя (которые основаны на originalLayer)
    ...
    if (['rect', 'image', 'text', 'pdf', 'parallelogram', 'parallelepiped'].includes(layer.type)) {
        ...
        ['p1', 'p2', 'p3', 'p4'].forEach(key => {
            if (layer[key]) layer[key] = applyOffset(originalLayer[key], { dx, dy });
        });
        ...
        if (layer.apex) layer.apex = applyOffset(originalLayer.apex, { dx, dy });
    } else if (layer.type === 'path') {
        layer.points = originalLayer.points.map(p => applyOffset(p, { dx, dy }));
    } else if (layer.type === 'curve') {
        layer.nodes.forEach((node, i) => {
            const originalNode = originalLayer.nodes[i];
            node.p = applyOffset(originalNode.p, { dx, dy });
            if (node.h1) node.h1 = applyOffset(originalNode.h1, { dx, dy });
            if (node.h2) node.h2 = applyOffset(originalNode.h2, { dx, dy });
        });
    } else if (layer.type === 'line') {
        layer.x1 = originalLayer.x1 + dx; layer.y1 = originalLayer.y1 + dy;
        layer.x2 = originalLayer.x2 + dx; layer.y2 = originalLayer.y2 + dy;
        ...
        ['base', 'top'].forEach(part => {
            if (layer[part]) {
                Object.keys(layer[part]).forEach(key => {
                    layer[part][key] = applyOffset(originalLayer[part][key], { dx, dy });
                });
            }
        });
    }

    // 6. Пересчитываем новый центр объекта (уже смещенного)
    const newCenterX = originalCenterX + dx;
    const newCenterY = originalCenterY + dy;

    // 7. Устанавливаем новый пивот как смещение от нового центра объекта к позиции курсора
    layer.pivot = {
...
export function handleEditCurve(state, pos, event) {
    ...
    if (handleInfo.type === 'curveNode') {
        // Перемещаем основной узел и его маркеры
        node.p.x = originalNode.p.x + dx;
        node.p.y = originalNode.p.y + dy;
        if (node.h1) {
            node.h1.x = originalNode.h1.x + dx;
