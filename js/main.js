...
function drawSelectionBox(ctx, selectedLayers, canvasState) {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Рисуем новую иконку для перемещения ---
    const moveHandleRadius = scaledHandleSize * 0.8;
    const moveHandlePos = { x: box.x + box.width + 15 / zoom, y: box.y + box.height + 15 / zoom };
    ctx.fillStyle = '#007AFF';
    ctx.strokeStyle = '#FFFFFF';
    ctx.lineWidth = scaledLineWidth * 1.5;

    ctx.beginPath();
    ctx.arc(moveHandlePos.x, moveHandlePos.y, moveHandleRadius, 0, 2 * Math.PI);
    ctx.fill();
    ctx.stroke();

    const arrowLength = moveHandleRadius * 0.5;
    ctx.strokeStyle = '#FFFFFF';
    ctx.lineWidth = scaledLineWidth * 2;
    ctx.beginPath();
    // Вертикальная стрелка
    ctx.moveTo(moveHandlePos.x, moveHandlePos.y - arrowLength);
    ctx.lineTo(moveHandlePos.x, moveHandlePos.y + arrowLength);
    // Горизонтальная стрелка
    ctx.moveTo(moveHandlePos.x - arrowLength, moveHandlePos.y);
    ctx.lineTo(moveHandlePos.x + arrowLength, moveHandlePos.y);
    ctx.stroke();
    // --- КОНЕЦ ИЗМЕНЕНИЙ ---
