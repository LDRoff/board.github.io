...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupCanvases = () => { 
        ...
        // --- ИЗМЕНЕНИЕ: Настройка HiDPI для всех холстов ---
        [backgroundCanvas, drawingCanvas, interactionCanvas, cacheCanvas].forEach(c => {
            // Физический размер (больше логического на Retina)
            c.width = width * dpr;
            c.height = height * dpr;
            
            // Видимый CSS размер (логический)
    ...
    // ... (updateSubToolbarVisibility, loadState, checkUiLayout и другие функции без изменений, скопируйте их из предыдущего файла) ...
    ...
    checkUiLayout();
    // ... (остаток функции с initializeFloatingTextToolbar и т.д.) ...
    function initializeFloatingTextToolbar() {
        // ... (содержимое функции остается прежним) ...
        // Код слишком длинный, чтобы дублировать полностью, он не меняется
        // кроме того, что есть в main.js в предыдущем ответе.
        // ВСТАВЬТЕ СЮДА ВЕСЬ ОСТАЛЬНОЙ КОД ИЗ main.js ПРЕДЫДУЩЕГО ЭТАПА
