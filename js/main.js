...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Добавляет изменение в историю и обновляет UI.
     * @param {object} change - Объект изменения {type, before, after}.
     */
    function commitChange(change) {
        history.addHistoryEntry(change, canvasState.layers);
        updateUndoRedoButtons();
        updateObjectCounter();
    }

    /**
     * Отложенная версия commitChange. Группирует быстрые вызовы в один.
     */
    function debouncedCommitChange(change) {
    ...
    function debouncedSaveViewState() {
        ...
        viewStateSaveTimer = setTimeout(() => {
            if (canvasState) {
                // Вызываем новую, безопасную функцию для сохранения только в БД, не трогая историю.
                history.scheduleDirectDBUpdate(canvasState.layers);
            }
        }, 500);
    }
    ...
    const setupCanvases = () => { 
        ...
        [backgroundCanvas, drawingCanvas, interactionCanvas, cacheCanvas].forEach(c => { c.width = width; c.height = height; }); 
    ...
    canvasState = initializeCanvas(drawingCanvas, interactionCanvas, ctx, redraw, commitChange, updateSubToolbarVisibility, debouncedSaveViewState, debouncedCommitChange);
    history.setCanvasStateRef(canvasState);
    canvasState.redraw = redraw;

    const eventHandlers = {
        performUndo, performRedo, commitChange, debouncedCommitChange,
        redraw, setupCanvases, updateSubToolbarVisibility,
        performDeselect: () => {
            if (canvasState.selectedLayers.length > 0) {
                // Deselection is a UI state, not a data change, so no history entry.
                canvasState.selectedLayers = [];
                canvasState.selectedCurveNodeIndex = null;
                redraw();
                canvasState.updateFloatingToolbar();
