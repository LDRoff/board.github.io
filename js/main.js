...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateSubToolbarVisibility() {
        ...
        const toolsWithSubToolbar = ['brush', 'text', 'rect', 'ellipse', 'line', 'parallelogram', 'triangle', 'trapezoid', 'rhombus', 'sphere', 'cone', 'parallelepiped', 'pyramid', 'frustum', 'truncated-sphere', 'truncated-pyramid'];
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Подключаем кнопки зума ---
    ...
    function applyAndSaveSettings() { const theme = themeSelect.value; const backgroundStyle = backgroundStyleSelect.value; document.body.classList.toggle('dark-theme', theme === 'dark'); localStorage.setItem('boardTheme', theme); localStorage.setItem('boardBackgroundStyle', backgroundStyle); redraw(); }
    function loadSettings() { const savedTheme = localStorage.getItem('boardTheme') || 'light'; const savedStyle = localStorage.getItem('boardBackgroundStyle') || 'dot'; themeSelect.value = savedTheme; backgroundStyleSelect.value = savedStyle; document.body.classList.toggle('dark-theme', savedTheme === 'dark'); redraw(); }
...
function drawLayer(ctx, layer) {
    ...
    else if (layer.type === 'text') { ctx.font = layer.font; ctx.textBaseline = 'top'; ctx.fillText(layer.text, layer.x, layer.y); }
    ctx.restore();
}
