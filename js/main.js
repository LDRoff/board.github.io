...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function performDeleteSelectedCurveNode() {
        ...
        // Нельзя удалять узлы, если их осталось 2 или меньше
        if (curve.nodes.length <= 2) return;
    
        curve.nodes.splice(canvasState.selectedCurveNodeIndex, 1);
        
        // Сглаживаем кривую заново после удаления
        utils.smoothCurveHandles(curve.nodes);
    
        canvasState.selectedCurveNodeIndex = null; // Сбрасываем выделение
        performSaveState(canvasState.layers);
        redraw();
        canvasState.updateFloatingToolbar();
    }
    ...
    function updateSubToolbarVisibility() {
        ...
        if (isDrawingContext) {
            ...
            if (hasSelection) {
                ...
                // Если ничего не выделено, показываем текущие настройки кисти
                document.getElementById('lineWidthSlider').value = canvasState.activeLineWidth;
    ...
    async function loadState(projectData = null) {
        ...
        if (!dataToParse) {
            // Если в хранилище ничего нет, инициализируем пустую историю
            history.resetHistory();
            performSaveState([], true); // Сохраняем пустое состояние
            return;
        }
        ...
        try {
            ...
            // Сначала применяем положение камеры
            if (viewState) {
            ...
            // ЖДЕМ, пока все картинки и PDF не будут полностью загружены
            const liveLayers = await utils.rehydrateLayers(layersToLoad);
            
            // ТОЛЬКО ПОСЛЕ ЭТОГО обновляем состояние приложения
            canvasState.layers = liveLayers;
            
            // Инициализируем историю ПРАВИЛЬНЫМ, полностью загруженным состоянием
            history.resetHistory();
            performSaveState(canvasState.layers, true);
            
            // И перерисовываем холст
            redraw();

        } catch (e) {
            console.error("Не удалось загрузить состояние:", e);
            history.resetHistory();
            performSaveState([]); // В случае ошибки сбрасываем состояние
        }
    }
    ...
    canvasState = initializeCanvas(drawingCanvas, interactionCanvas, ctx, redraw, performSaveState, updateSubToolbarVisibility, debouncedSaveViewState);
    history.initHistory(canvasState);
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Явно добавляем redraw в canvasState ---
                ...
                if (!wasActive) {
                    ...
                    if (dropdownMenu) {
                        ...
                        if (spaceBelow < menuHeight + margin && spaceAbove > menuHeight + margin) {
                            // Места снизу нет, но есть сверху - открываем вверх
                            // (это поведение по умолчанию в CSS, класс не нужен)
                        } else {
    ...
    function initializeCustomTooltips() {
        ...
        document.body.addEventListener('mouseover', (e) => {
            ...
            if (targetRect.top < tooltipHeight + margin + 5) {
                // Недостаточно места сверху, переворачиваем вниз
