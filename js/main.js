...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadState(projectData = null) {
        ...
        try {
                ...
                layersToLoad.forEach(layer => {
                    if (layer.type === 'image' && layer.src) {
                        const promise = new Promise((resolve, reject) => {
                            const img = new Image();
                            img.onload = () => { layer.image = img; delete layer.src; resolve(); };
                            img.onerror = reject;
                            img.src = layer.src;
                        });
                        ...
                        const promise = new Promise(async (resolve, reject) => {
                            try {
                                ...
                                // Рендерим текущую страницу после загрузки
                                await utils.renderPdfPageToCanvas(layer, layer.currentPage);
                                
                                delete layer.fileData;
                                resolve();
                            } catch (err) {
            ...
            Promise.all(loadPromises).then(() => {
                ...
                console.error("Не все ресурсы удалось загрузить.");
                canvasState.layers = layersToLoad.filter(l => (l.type !== 'image' && l.type !== 'pdf') || l.image || l.pdfDoc);
                history.resetHistory();
                performSaveState(canvasState.layers);
                redraw();
            });
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Полностью переписанная функция для корректного позиционирования подсказок ---
