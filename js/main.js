...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeFloatingTextToolbar() {
        ...
        toolbar.addEventListener('click', e => {
            ...
            // --- НАЧАЛО ИЗМЕНЕНИЙ: Улучшена логика открытия выпадающих окон ---
            if (Object.keys(dropdownActionMap).includes(action)) {
                e.stopPropagation();
                const container = dropdownActionMap[action];
                const wasActive = container.classList.contains('active');
                
                // Сначала закрываем все выпадающие окна и сбрасываем их положение
                dropdowns.forEach(d => {
                    d.classList.remove('active');
                    const menu = d.querySelector('.dropdown-options, .floating-palette');
                    if (menu) menu.classList.remove('opens-downward');
                });
                
                // Если окно не было активно, открываем его и вычисляем положение
                if (!wasActive) {
                    // Делаем контейнер активным, чтобы меню стало видимым для измерений
                    ...
                    if (dropdownMenu) {
                        ...
                        // По умолчанию открывается вверх. Если сверху места нет, а снизу есть - открываем вниз.
    ...
    function initializeSettingsModal() {
        ...
        // --- НАЧАЛО ИЗМЕНЕНИЙ: Логика для кастомных выпадающих списков ---
        ...
        const settingsDropdowns = [themeDropdownContainer, bgDropdownContainer];
        ...
        settingsModal.addEventListener('click', (e) => {
            if (!themeDropdownContainer.contains(e.target) && !bgDropdownContainer.contains(e.target)) {
        ...
        function loadSettings() { 
            ...
            // --- НАЧАЛО ИЗМЕНЕНИЙ: Обновление кастомных выпадающих списков ---
        ...
        function closeModal() {
            ...
            settingsDropdowns.forEach(d => d.classList.remove('active')); // Закрываем дропдауны
        }
