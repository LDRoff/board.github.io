...
export function getLayerAtPosition(pos, layers, zoom, spatialGrid, eraserRadius = 0, onlyOutline = false) {
    ...
    for (let i = candidates.length - 1; i >= 0; i--) {
        ...
        if (layer.type === 'path') {
            ...
            const checkOutline = onlyOutline && !layer.fillColor; // If filled, you can erase by clicking the fill
            ...
            hit = geo.isPointInEllipse(rotatedPos, { cx: layer.cx, cy: layer.cy, rx, ry, lineWidth: layer.lineWidth }, eraserRadius, onlyOutline && !layer.fillColor);
        }
        else if (layer.type === 'ellipse') {
            hit = geo.isPointInEllipse(rotatedPos, layer, eraserRadius, onlyOutline && !layer.fillColor);
        }
            ...
            hit = geo.isPointInTriangle(rotatedPos, layer.p1, layer.p2, layer.p3, eraserRadius, onlyOutline && !layer.fillColor, layer.lineWidth || 2);
        }
            ...
            hit = geo.isPointInPolygon(rotatedPos, [layer.p1, layer.p2, layer.p3, layer.p4], eraserRadius, onlyOutline && !layer.fillColor, layer.lineWidth || 2);
        }
            ...
            for (const face of faces) {
                if (geo.isPointInPolygon(rotatedPos, face, eraserRadius, onlyOutline && !layer.fillColor, layer.lineWidth || 2)) {
                    hit = true;
                    break;
                }
            }
