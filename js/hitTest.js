...
export function getLayerAtPosition(pos, layers) { 
    for (let i = layers.length - 1; i >= 0; i--) { 
        ...
        if (layer.type === 'path') { hit = geo.isPointOnPath(rotatedPos, layer); } 
            ...
            // --- НАЧАЛО ИЗМЕНЕНИЙ: Исправлена логика для использования предварительно вычисленного 'box' ---
            // Ранее здесь был повторный вызов getBoundingBox, что вызывало ошибку для PDF.
...
export function getHandleAtPosition(pos, layers, zoom, groupRotation) {
    ...
    // Рендеринг использует ctx.rotate(), который вращает ПО часовой стрелке.
    // Чтобы это компенсировать, мы должны вращать точку ПРОТИВ часовой стрелки.
    const cos = Math.cos(rotation);
    const sin = Math.sin(rotation);
    const dx = pos.x - pivotX;
    const dy = pos.y - pivotY;

    // Формула вращения ПРОТИВ часовой стрелки
    const rotatedPos = {
        x: dx * cos + dy * sin + pivotX,
        y: -dx * sin + dy * cos + pivotY
    };
    ...
    // Маркер вращения (расположен под правым нижним углом)
    ...
    for (const handleName in handles) {
        ...
        if (rotatedPos.x >= handlePos.x - halfHandle && rotatedPos.x <= handlePos.x + halfHandle && 
            rotatedPos.y >= handlePos.y - halfHandle && rotatedPos.y <= handlePos.y + halfHandle) {
            return handleName;
        }
    }
