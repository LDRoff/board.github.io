...
export function getLayerAtPosition(pos, layers, zoom, spatialGrid) { 
    // Используем пространственную сетку для получения только "близких" кандидатов,
    // если сетка доступна. В противном случае (fallback) перебираем все слои.
    const candidates = spatialGrid ? getLayersInProximity(spatialGrid, pos) : layers;
    ...
    for (let i = candidates.length - 1; i >= 0; i--) { 
        ...
        let hit = false; 
        if (layer.type === 'path') { 
            hit = geo.isPointOnPath(rotatedPos, layer, zoom); 
        } 
        else if (layer.type === 'curve') { 
            hit = geo.isPointOnCurve(rotatedPos, layer, zoom); 
        }
        else if (layer.type === 'text' || layer.type === 'rect' || layer.type === 'image' || layer.type === 'pdf') {
            ...
            hit = geo.isPointInEllipse(rotatedPos, layer); 
        } 
        else if (layer.type === 'line') { 
            hit = geo.isPointOnLineSegment(rotatedPos, layer, zoom); 
        } 
        else if (layer.type === 'parallelogram') { 
            hit = geo.isPointInParallelogram(rotatedPos, layer); 
        } 
        else if (layer.type === 'triangle') { 
            hit = geo.isPointInTriangle(rotatedPos, layer.p1, layer.p2, layer.p3); 
        } 
        else if (layer.type === 'cone') { 
            hit = geo.isPointInCone(rotatedPos, layer); 
        } 
        else if (layer.type === 'parallelepiped') { 
            hit = geo.isPointInParallelepiped(rotatedPos, layer); 
        } 
        else if (layer.type === 'pyramid') { 
            hit = geo.isPointInPyramid(rotatedPos, layer); 
        } 
        else if (layer.type === 'trapezoid' || layer.type === 'rhombus') { 
            hit = geo.isPointInPolygon(rotatedPos, [layer.p1, layer.p2, layer.p3, layer.p4]); 
        } 
        else if (layer.type === 'frustum') { 
            hit = geo.isPointInFrustum(rotatedPos, layer); 
        } 
            ...
            for (const face of faces) { 
                if (geo.isPointInPolygon(rotatedPos, face)) { 
                    hit = true; 
                    break; 
                } 
            } 
