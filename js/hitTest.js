...
export function getHandleAtPosition(pos, layers, zoom, groupRotation) {
    ...
    // --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем проверку для новой иконки перемещения ---
    const moveHandlePos = { x: box.x + box.width + 15 / zoom, y: box.y + box.height + 15 / zoom };
    const rotatedMoveHandle = geo.rotatePoint(moveHandlePos, { x: pivotX, y: pivotY }, rotation);
    const moveHandleHitbox = (10 / zoom) * 1.2; // Делаем hitbox немного больше для удобства
    if (pos.x >= rotatedMoveHandle.x - moveHandleHitbox && pos.x <= rotatedMoveHandle.x + moveHandleHitbox &&
        pos.y >= rotatedMoveHandle.y - moveHandleHitbox && pos.y <= rotatedMoveHandle.y + moveHandleHitbox) {
        return 'move';
    }
    // --- КОНЕЦ ИЗМЕНЕНИЙ ---
