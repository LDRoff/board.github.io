...
export function getLayerAtPosition(pos, layers, zoom, spatialGrid) { 
    ...
    for (let i = candidates.length - 1; i >= 0; i--) { 
        ...
        else if (['text', 'rect', 'image', 'pdf'].includes(layer.type)) {
 ...
 */
export function getLayerForEraser(pos, layers, zoom, spatialGrid, bboxCache) {
    ...
    // Размер "пятна" ластика
    const eraserRadius = 12 / zoom;
    const eraserRect = {
    ...
    for (let i = candidates.length - 1; i >= 0; i--) {
        ...
        const hitLayer = getLayerAtPosition(pos, [layer], zoom, null);
        if (hitLayer) {
            return hitLayer;
        }
    }
